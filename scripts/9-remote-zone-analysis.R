...
# VlnPlots
for (i in genes) {
  pdf(file = paste0("results/remote-zone-analysis/VlnPlot_", i,".pdf"),
      height = 6,
      width = 8,
      useDingbats = F)
  p <- VlnPlot(hearts, 
...
a <- SpatialDimPlot(hearts, 
                    ...
                    stroke = NA) + 
  scale_fill_manual(values= c("#F4BA27", "#ff00ff00"),
                    labels=c("Remote", "rest")) +
  NoLegend() +
  labs(title = "Remote zone", subtitle = "Sham - 24 h") +
  theme(plot.title = element_text(color = "#F4BA27",
                                  size = 16,
                                  face = "bold"),
...
b <- SpatialDimPlot(hearts, 
  ...
  theme(plot.title = element_text(color = "#F4BA27",
                                  ...
                                  face = "bold"),
        plot.subtitle = element_text(size = 14))
pdf(file = "results/remote-zone-analysis/SpatialDimPlot_remote_zone.pdf", 
    height = 6, 
    width = 11.25, 
    useDingbats = F)
print(a + b)
...
pdf(file = paste0("results/remote-zone-analysis/BoxPlot_lipid_catabolic_process.pdf"),
    height = 6,
    width = 8,
    useDingbats = F)
ggplot(remote_zone_data,aes(x = surgery,
